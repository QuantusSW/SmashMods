...
nut = open(sys.argv[2], "rb+")
mainID = int(sys.argv[3])
fighterID = int(sys.argv[4])
texID = int(sys.argv[5])
offsetID = int(sys.argv[6])
texID
#setup are arrays
boneCount = []
vertexStart_array=[]
...
for i in range(fileTotal):
        ...
        if(texNum > 0 and texNum <= 127):
                ...
                texNum = texNum - 128
        nut.seek(-4,1)
        nut.write(struct.pack("B", mainID))
        nut.write(struct.pack("B", fighterID))
        nut.write(struct.pack("B", texID))
        nut.write(struct.pack("B", offsetID + texNum))
        nut.seek(-4,1)
        ...
        nut.seek(4,1)
        print("%i %i %i %i" %(typeNum,fightNum,skinNum,fileNum))
nut.close()
